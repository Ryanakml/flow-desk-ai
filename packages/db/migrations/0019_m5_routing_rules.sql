...
CREATE TABLE IF NOT EXISTS flowdesk.routing_rules (
    ...
    target_queue_id uuid REFERENCES flowdesk.queues(id) ON DELETE SET NULL,
    target_team_id uuid REFERENCES flowdesk.teams(id) ON DELETE SET NULL,
...
CREATE POLICY tenant_isolation_routing_rules ON flowdesk.routing_rules
    FOR ALL
    TO PUBLIC
    USING (organization_id = flowdesk.current_tenant_id())
    WITH CHECK (organization_id = flowdesk.current_tenant_id());
